function varargout = July09_v2(varargin)
% July09_v2 MATLAB code for July09_v2.fig
%      July09_v2, by itself, creates a new July09_v2 or raises the existing
%      singleton*.
%
%      H = July09_v2 returns the handle to a new July09_v2 or the handle to
%      the existing singleton*.
%
%      July09_v2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in July09_v2.M with the given input arguments.
%
%      July09_v2('Property','Value',...) creates a new July09_v2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before July09_v2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to July09_v2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help July09_v2

% Last Modified by GUIDE v2.5 10-Apr-2019 15:04:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @July09_v2_OpeningFcn, ...
                   'gui_OutputFcn',  @July09_v2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before July09_v2 is made visible.
function July09_v2_OpeningFcn(hObject, ~, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to July09_v2 (see VARARGIN)

% Choose default command line output for July09_v2
addpath ./MClust-3.5/Utilities
addpath ./backup_fake_data
...
% UIWAIT makes July09_v2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = July09_v2_OutputFcn(~, ~, handles) 
...
switch get(eventdata.NewValue,'Tag')
    ...
    set(handles.MRI_togglebutton,'value',0);
    set(handles.PET_togglebutton,'value',1);
    set(handles.PET_togglebutton,'value',0);
    set(handles.MRI_togglebutton,'backgroundcolor',[0,0,0]);
    set(handles.PET_togglebutton,'backgroundcolor',[0.8,0.8,0.8]);
    set(handles.PET_PVC_togglebutton,'backgroundcolor',[0,0,0]);
...
try
    ...
    if(gca==handles.axes4)
        ...
        pos = getPosition(h2); 
        pos=sort(round(pos(:,2)/PET.voxel_size(3))+1);
...
set(handles.PET_togglebutton,'value',0);
set(handles.PET_PVC_togglebutton,'value',0);
set(handles.MRI_togglebutton,'backgroundcolor',[0.8,0.8,0.8]);
set(handles.PET_togglebutton,'backgroundcolor',[0,0,0]);
set(handles.PET_PVC_togglebutton,'backgroundcolor',[0,0,0]);
set(handles.MRI_panel,'visible','on');
...
Message_State(handles);
    ...
    set(handles.PET_panel,'visible','on');
    set(handles.PET_PVC_panel,'visible','off');
    set(handles.X_Value,'string',num2str(round(size(PET.Matrix,1)/2)));
    set(handles.Y_Value,'string',num2str(round(size(PET.Matrix,2)/2)));
    set(handles.Z_Value,'string',num2str(round(size(PET.Matrix,3)/2)));
    set(handles.MIP,'value',0);
    X_Value=round(str2double(get(handles.X_Value,'string')));
    Y_Value=round(str2double(get(handles.Y_Value,'string')));
    Z_Value=round(str2double(get(handles.Z_Value,'string')));
...
[ grey_level_mean,entropy,uniformity,Id ] = Heterogeneity_CT_Guanfeng_Revise( [PathName,FileName] );
mydialog(['The grey_level_mean of the ROI is:', num2str(grey_level_mean),' ,The entropy of the ROI is:'...
    ,num2str(entropy),' ,The uniformity of the ROI is:',num2str(uniformity),' ,The Id of the ROI is:', num2str(Id)],0);


% --- If Enable == 'on', executes on mouse press in 5 pixel border.
% --- Otherwise, executes on mouse press in 5 pixel border or over PET_togglebutton.
function PET_togglebutton_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to PET_togglebutton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.PET_panel,'visible','on');
set(handles.PET_PVC_panel,'visible','off');
...
% --------------------------------------------------------------------
function DIP_PVC_Callback(hObject, eventdata, handles)
% hObject    handle to DIP_PVC (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
% --------------------------------------------------------------------
function test_network_Callback(hObject, eventdata, handles)
% hObject    handle to test_network (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.Inputpanel,'visible','off');
set(handles.Displaypanel,'visible','off');
set(handles.Messagepanel,'visible','off');
set(handles.DIPpanel, 'Parent', gcf);
set(handles.DIPpanel,'visible','on');


% --- Executes on button press in Img_File.
function Img_File_Callback(hObject, eventdata, handles)
% hObject    handle to Img_File (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Img_name, Img_path]=uigetfile(pwd,'Please select a file');
set(handles.ImgName,'string',[Img_path, Img_name]);


% --- Executes on button press in Bp_File.
function Bp_File_Callback(hObject, eventdata, handles)
% hObject    handle to Bp_File (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Bp_name, Bp_path]=uigetfile(pwd,'Please select a file');
set(handles.BpName,'string',[Bp_path, Bp_name]);


% --- Executes on button press in Run_TestNetwork.
function Run_TestNetwork_Callback(hObject, eventdata, handles)
% hObject    handle to Run_TestNetwork (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[Img_File]=get(handles.ImgName,'string');
[Bp_File]=get(handles.BpName,'string');
[OutputDir]=get(handles.OutputName,'string');
[TrainDir]=get(handles.TrainName,'string');
python('DeepImagePriorCode/test_network_structure_leakyrelu_3d_compz_nomomen_BP_noshrink_init.py', [Img_File], [Bp_File], [OutputDir], [TrainDir])
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[OutputDir]=uigetdir(pwd,'Please select a file');
set(handles.OutputName,'string',[OutputDir]);


% --- Executes on button press in Train_File.
function Train_File_Callback(hObject, eventdata, handles)
% hObject    handle to Train_File (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[TrainDir]=uigetdir(pwd,'Please select a file');
set(handles.TrainName,'string',[TrainDir]);




% --------------------------------------------------------------------
function InputItem_Callback(hObject, eventdata, handles)
% hObject    handle to InputItem (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
set(handles.Inputpanel,'visible','on');
set(handles.Displaypanel,'visible','on');
set(handles.Messagepanel,'visible','on');
set(handles.DIPpanel,'visible','off');
