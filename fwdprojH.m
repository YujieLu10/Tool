function Iout = fwdprojH(Iin,params)
% H,D,Iin,sizex)
...
%     idx3=floor((sizex(3)-sizex(3)/2+1):(sizex(3)+sizex(3)/2));
    idx1=floor((sizex(1)-sizex(1)/2):(sizex(1)+sizex(1)/2-1));
    idx2=floor((sizex(2)-sizex(2)/2):(sizex(2)+sizex(2)/2-1));
    idx3=floor((sizex(3)-sizex(3)/2):(sizex(3)+sizex(3)/2-1));
